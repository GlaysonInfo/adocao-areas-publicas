 ...
 *
 * Objetivos:
 * - evitar strings soltas espalhadas pelo código
 * - facilitar validação / autocomplete
 * - preparar mapeamentos de legado -> modelo atual
 */
...
/**
 * Mapeia valores antigos/legados para o modelo canônico atual do MVP.
 * Útil para importações, migração de localStorage e convivência temporária com chaves antigas.
 */
export function normalizeLegacyAreaStatus(raw: unknown): AreaStatus | null {
  ...
  if (s === "adotada" || s === "termo_assinado") return "adotada";
  return null;
}
...
export function normalizeLegacyProposalStatus(raw: unknown): KanbanColuna | null {
  ...
  // legados fora do fluxo atual, mas que podem ser tratados como entrada inicial
